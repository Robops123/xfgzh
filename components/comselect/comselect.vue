    ...
    <view class="min-action" :class="{'min-action-show': show}" @touchmove.stop.prevent="()=>{}">
        ...
        <view class="min-action-main" :class="{'min-action-main-show': show}">
            ...
            <view class="min-action-cancel" v-if="showCancel">
                <view class="min-action-flex min-action-btn" 
                    @click="handleClick(-1)">
                    {{cancelText}}
                </view>
            </view>
        ...
        handleClick (id) {
            ...
            if (this.actions[id]) {
                if (this.actions[id].loading === 0) {
                    ...
                    this.success({asID: this.asID, id, handleHide: this.handleHide,devName:this.actions[id].name})
                    this.isClick = false
                    return
                }
            }
            this.success({asID: this.asID, id,devName:this.actions[id].name})
            this.handleHide()
        }
    }
}
